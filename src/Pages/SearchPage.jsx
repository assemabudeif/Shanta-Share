...
export default function SearchPage() {
  ...
  const handleSubmit = (e) => {
      ...
      setCurrentPage(1);
      GetPosts({...searchForm,
        max_size: searchForm.max_height * searchForm.max_width * searchForm.max_depth / 1000000,
        page: currentPage
      });
  }
  ...
  useEffect(() => {
    GetPosts({...searchForm, page: currentPage});
  }, [currentPage]);
  useEffect(() => {
    if (searchForm.from_government) {
      // Fetch cities based on the selected government
      fetch(`http://localhost:8000/cities?government_id=${searchForm.from_government}`)
        .then(response => response.json())
        .then(data => setPickupCities(data))
        .catch(error => console.error('Error fetching cities:', error));
    }
  }, [searchForm.from_government]);
  useEffect(() => {
    if (searchForm.to_government) {
      // Fetch cities based on the selected government
      fetch(`http://localhost:8000/cities?government_id=${searchForm.to_government}`)
        .then(response => response.json())
        .then(data => setArrivalCities(data))
        .catch(error => console.error('Error fetching cities:', error));
    }
  }, [searchForm.to_government]);
        ...
        <div className={"border-2 border-black rounded-2xl p-8"}>
          ...
          <div>
            ...
            <select
              ...
              value={searchForm.from_government}
              onChange={handleChange}
              className="
                          bg-[#F3F3F3]
          ...
          <div>
            ...
            <select
              name="to_government"
              value={searchForm.to_government}
              onChange={handleChange}
              className="
                          bg-[#F3F3F3]
                          block w-full rounded-md
            ...
            posts.map((post, index) => (
              <div className={"border-2 border-black rounded-2xl grid grid-cols-4 py-8"}>
                ...
                <div className={"col-span-3"}>
                  <div
                    className={"flex flex-col lg:flex-row justify-between items-start lg:items-center me-8 mb-4"}>
                    <div className='mb-4 lg:mb-0'>
                                        <span className={"text-2xl font-semibold"}>
                                            {post.from_city ? post.from_city.name + ',' : ''} {post.from_city ? post.from_city.government.name : ''}
                    ...
                    <div>
                                        <span className={"text-2xl font-semibold"}>
                                           {post.to_city ? post.to_city.name + ',' : ''} {post.to_city ? post.to_city.government.name : ''}
                                          <span className={"text-xl font-normal"}>
                  ...
                  <div>
                    <div className="flex items-center justify-start mt-2">
                      ...
                      <span
                        className="ms-2 text-black-600 font-semibold me-8">{post.created_by.average_rating}</span>
                      {
                        post.verified && (
                          <>
                            <svg width="24" height="24" viewBox="0 0 24 24" fill="none"
                    ...
                    <h3 className={"text-xl font-semibold mt-2"}>
                      {post.max_weight}kg max {post.max_size} msq
                    </h3>
                    <div className={"flex  justify-between"}>
                      <h3 className={"text-xl font-semibold mt-2"}>
                        Truck
