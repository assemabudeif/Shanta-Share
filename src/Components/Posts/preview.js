  ...
  return (
    <div className="p-6 bg-white-50 shadow-md rounded-lg space-y-4 preview flex flex-col justify-between h-full">
      ...
      <div className="space-y-2">
        ...
        <p className='color-text'>{formData.from_address_line|| 'Address line'}, {formData.from_city?.name ?? 'City'}, {formData.from_government?.name ?? 'Government'}</p>
        <p className='color-text'>{formData.pickup_time || 'DD/MM/YYYY - HH:MM'}</p>
      </div>
      <div className="space-y-2">
        <strong className="block">Destination</strong>
        <p className='color-text'>{formData.to_address_line|| 'Address line'}, {formData.to_city?.name ?? 'City'}, {formData.to_government?.name ?? 'Government'} </p>
      ...
      <div className="mt-auto space-y-4">
        <div className="flex items-center space-x-2">
          ...
          <button class={`rounded-full py-2 px-4  text-white w-1/3 ${!isFormReady ? 'bg-black' : 'bg-gray-500'} `} onClick={handleCalculateFEEs} disabled={isFormReady}>Calculate Price</button>
          {/* <button className="w-full py-2 px-4 bg-black text-white rounded">Calculate Price</button> */}
        </div>
        <hr />
        <button className={`w-full py-2 px-4  text-white rounded-full ${isFormReady ? 'bg-black' : 'bg-gray-500'}  `} disabled={!isFormReady} onClick={onSubmit}>Confirm Post</button>
      </div>
    </div>
  );
}
